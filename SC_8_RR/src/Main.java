...
{ 
	...
	public void schedule() {
        ...
        for (Process p : readyQueue) {
            if (p.getServiceTime() < ShortestServiceTime) {
                ShortestProcess = p;
                ShortestServiceTime = p.getServiceTime();
            }
        }
        // 찾은 프로세스를 currentProcess에 설정
...
class HRRN extends Scheduler{
	...
	public void schedule() {
		// TODO Auto-generated method stub
		super.schedule();
		Process HighSpeedProcess = null;
		double HighSpeedRatio = Double.MIN_VALUE;
		
		for(Process p : readyQueue) {
			double ResponseRatio = p.getResponeRatioTime(currentTime);
			if(ResponseRatio > HighSpeedRatio) {
				HighSpeedProcess = p;
				HighSpeedRatio = ResponseRatio;
			}
		}currentProcess = HighSpeedProcess;
...
class SRT extends Scheduler{
	...
	public SRT(String name) {
        super(name);
    }

    @Override
    public void schedule() {
        super.schedule(); // 슈퍼 클래스의 schedule() 호출

        // readyQueue에 있는 프로세스 중 가장 작은 Remaining Time을 가지는 프로세스 찾기
        Process KTXProcess = null;
        
        int KTXRemainingTime = Integer.MAX_VALUE;

        for (Process p : readyQueue) 
        {
            if (p.getRemainingTime() < KTXRemainingTime)
            {
                KTXProcess = p;
                KTXRemainingTime = p.getRemainingTime();
            }
        }

        // 찾은 프로세스를 currentProcess에 설정
        currentProcess = KTXProcess;
    }
...
class RR extends Scheduler {
    ...
    @Override
    public void schedule() {
        super.schedule(); // 부모 클래스의 schedule 메소드 호출

        if (currentProcess != null) {
            quantum--; // 현재 프로세스가 실행될 때마다 quantum 감소

            if (quantum == 0) {
                readyQueue.offer(currentProcess); // 현재 실행 중인 프로세스를 다시 레디 큐에 추가
                currentProcess = readyQueue.poll(); // 레디 큐의 맨 앞 프로세스를 실행 대기로 설정
                quantum = getInitialQuantum(); // 퀀텀 초기화
            }
        }
    }
    ...
    @Override
    public boolean isSchedulable() {
        return super.isSchedulable() || (currentProcess != null && quantum == 0);
    }
   ...
   public  void clockInterrupt() {
    ...
    private int getInitialQuantum() {
        return quantum; // 이 메소드를 통해 초기 quantum을 얻을 수 있습니다.
    }
}
